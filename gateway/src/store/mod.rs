...
mod tests {
    ...
    async fn persists_and_replays() {
        ...
        store
            .write_header(&run("R1", RunStatus::Running))
            .await
            .unwrap();
        store
            .write_span(&Span {
        ...
        store
            ...
            .unwrap();
        store
            .update_run(&run("R1", RunStatus::Completed))
            .await
            .unwrap();
    ...
    async fn index_orders_newest_first() {
        let dir = tempfile::tempdir().unwrap();
        let store = RunStore::new(dir.path()).unwrap();
        let mut a = run("A", RunStatus::Completed);
        a.started_at = "2026-05-31T00:00:01Z".into();
        let mut b = run("B", RunStatus::Completed);
        b.started_at = "2026-05-31T00:00:02Z".into();
